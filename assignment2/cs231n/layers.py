...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T).reshape(x.shape) #N,M * M,D  N,D  D=(d1*d2*d3)
    dw = x.reshape(N,D).T.dot(dout) #D,M * N,M  D,M
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_var = np.sqrt(np.var(x, axis = 0) + eps)
        x_norm = (x - sample_mean) / sample_var
        out = gamma * x_norm + beta

        running_mean = momentum * running_mean + (1 - momentum) * sample_mean 
        running_var = momentum * running_var + (1 - momentum) * sample_var

        cache = x, sample_var, sample_mean, x_norm, out, gamma, beta
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        x_norm = (x - running_mean) / (np.var(np.std(x, axis = 0) + eps))
        out = gamma * x_norm + beta
